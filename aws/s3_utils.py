...
def upload_server_app_to_s3_bucket(s3, bucket, to_archieve='../worker', archieve_name='worker') -> None:
    shutil.make_archive(archieve_name, 'zip', to_archieve)
    s3.upload_file(f'./{archieve_name}.zip', bucket, f'{archieve_name}.zip')
